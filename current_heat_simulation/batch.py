...
def execute_batch(shared, output, workers, resume=False):
    ...
    results, pending = [], []
    line_totals = {
        name: sum(task.source_lines for tasks in shared["days"].values() for task in tasks)
        for name in shared["layouts"]
    }
    bars = {
        name: tqdm(total=total, desc=name, unit=" lines", position=index, leave=True)
        for index, (name, total) in enumerate(line_totals.items())
    }
    completed_lines = {name: 0 for name in shared["layouts"]}
    ...
    def drain_progress():
        while True:
            ...
            if len(item) == 3 and item[1] == "time":
                name, _, seconds = item
                bars[name].set_postfix_str(f"sim {seconds:.0f}s", refresh=False)
            else:
                name, lines = item
                bars[name].update(int(lines))
                completed_lines[name] += int(lines)
    ...
    for name in shared["layouts"]:
        for day in sorted(shared["days"]):
            ...
            if checkpoint is not None:
                ...
                completed_lines[name] += int(checkpoint.get("completed_lines", 0))
    ...
    def collect(summary):
        ...
        print(f"[{len(results)}/{total}] "
              ...
              f"{summary.get('completed_lines', 0)} lines", flush=True)

    try:
        if worker_count <= 1:
            for name, day in pending:
            ...
            with ProcessPoolExecutor(max_workers=worker_count,
                ...
                futures = {pool.submit(_worker, name, day, output): (name, day) for name, day in pending}
                ...
                while pending_futures:
                    ...
                    for future in done:
                        try:
                            ...
                            collect({"layout": name, "date": day, "status": "failed", "success_flag": False,
                                     "failure_reason": f"worker failure: {type(exc).__name__}: {exc}"})
                    drain_progress()
    finally:
        for name, bar in bars.items():
            drain_progress()
            if bar.n < completed_lines[name]:
                bar.update(completed_lines[name] - bar.n)
            bar.close()
        manager.shutdown()
