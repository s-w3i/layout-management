...
DistTable::DistTable(const Instance &ins)
    : K(ins.G.V.size()), instance(&ins), table(ins.N, std::vector<int>(K, K))
{
  setup(&ins);
}
  ...
  if (MULTI_THREAD_INIT) {
    auto bfs = [&](const int i) {
      ...
      while (!Q.empty()) {
        ...
        for (auto &m : n->reverse_neighbors) {
          if (!ins->is_allowed(i, m)) continue;
...
int DistTable::get(const int i, const int v_id)
{
  ...
  while (!OPEN[i].empty()) {
    ...
    const int d_n = table[i][n->id];
    for (auto &&m : n->reverse_neighbors) {
      if (!instance->is_allowed(i, m)) continue;
      const int d_m = table[i][m->id];
      if (d_n + 1 >= d_m) continue;
      table[i][m->id] = d_n + 1;
      OPEN[i].push(m);
