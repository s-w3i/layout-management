...
bool PIBT::funcPIBT(const int i, const Config &Q_from, Config &Q_to)
{
  ...
  // emulate swap
  Vertex *preferred = Q_from[i];
  for (size_t k = 0; k < K + 1; ++k) {
    auto candidate = C_next[i][C_indices[i][k]];
    if (ins->is_allowed(i, candidate)) { preferred = candidate; break; }
  }
  const auto swap_agent = is_swap_required_and_possible(
      i, Q_from, Q_to, preferred);
  ...
  for (size_t k = 0; k < K + 1; ++k) {
    ...
    if (!ins->is_allowed(i, u)) continue;
